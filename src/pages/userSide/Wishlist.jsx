...
//             saveAndNotify(updated)   // ← localStorage + event dono
//             return updated
//         })
//     }
//     const clearAll = () => {
//         saveAndNotify([])   // ← pehle save karo
//         setItems([])
//     }
...
// ✅ Normalize category — trim + title case taake grouping consistent rahe
...
// ✅ Helper — localStorage update karo aur Navbar ko event bhejo
function saveAndNotify(updated) {
...
export default function Wishlist() {
    ...
    // ✅ FIX — normalize karo taake "Electronics" aur "electronics " alag na banein
    const categories = useMemo(() => {
        const seen = new Set()
        const result = []
    ...
    // ✅ FIX — filter bhi case-insensitive
    const filtered = useMemo(() => {
        return items.filter((i) => {
            const itemCat = normalizeCategory(i.category)
            const matchesCat =
                activeCat === 'All' ||
                itemCat.toLowerCase() === activeCat.toLowerCase()
            const q = query.trim().toLowerCase()
            const matchesQ =
                !q ||
    ...
    // ✅ localStorage sync + event dispatch dono
    const removeItem = (id) => {
        setItems((prev) => {
            const updated = prev.filter((i) => i.id !== id)
